...
class HandDetector:
    ...
    def detect_hands(self, frame: np.ndarray) -> Tuple[np.ndarray, Dict]:
        ...
        if results.multi_hand_landmarks:
            ...
            for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                handedness = self._handedness_label(results, idx)
                hand_info = self._extract_hand_info(
                    hand_landmarks, frame.shape, handedness
                )
    ...
    def _handedness_label(self, results, idx: int) -> str:
        if not results.multi_handedness or idx >= len(results.multi_handedness):
            return ''
        classification_list = results.multi_handedness[idx]
        if not classification_list.classification:
            return ''
        return classification_list.classification[0].label or ''

    def _extract_hand_info(
            self, landmarks, frame_shape, handedness: str = '') -> Dict:
